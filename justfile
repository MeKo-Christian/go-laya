...
# library (the ONNX backend forward pass loads 1.7 GB when build/onnx exists); `test-race`
...
# Run the ORT-backed tests (they skip unless an ORT library and the S1 exports exist)
test-onnx:
    LAYA_ONNX_DIR="${LAYA_ONNX_DIR:-{{justfile_directory()}}/build/onnx}" \
        CGO_ENABLED=0 go test -count=1 -v ./internal/backend/onnx/

# The S2.2 finalizer hunt. -race needs cgo, so this cannot also assert CGO_ENABLED=0.
test-onnx-race count="200":
    LAYA_ONNX_DIR="${LAYA_ONNX_DIR:-{{justfile_directory()}}/build/onnx}" \
        go test -race -count={{count}} ./internal/backend/onnx/
...
bench-onnx reps="5" out="build/bench-onnx.txt":
    ...
    LAYA_ONNX_DIR="${LAYA_ONNX_DIR:-{{justfile_directory()}}/build/onnx}" \
        ...
        -benchtime={{reps}}x -timeout 120m -v ./internal/backend/onnx/ 2>&1 | tee {{out}}
