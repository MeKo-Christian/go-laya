...
- [x] **6.8.1** All three checkpoints at S1's four shapes, against fixtures generated by
      `scripts/export_onnx.py --fixture-matrix`, using `--fixture`'s schema and the `maxScaledDiff`
      metric `internal/backend/onnx/forward_pass_test.go` implements (moved there from
      `internal/onnxspike/spike_test.go` by Task 6.10; do not re-derive).
      ...
      non-traced shapes. _(PR #19 review)_ — `maxScaledDiff` returned 0 for an all-NaN output,
      because a NaN delta compares false against the running worst. It now returns +Inf for any
      non-finite value on either side, so all three ORT parity tests reject it
      (`TestMaxScaledDiff`, which runs in CI).
