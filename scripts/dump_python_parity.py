...
# What produced the numbers. Section 8 of PLAN.md promises "within 1e-4 of PyTorch",
# and that tolerance is only meaningful against a stated device, dtype and attention
# implementation -- fp16 on a GPU under eager attention is a different claim entirely
# (task 1.7.2). The facts, not guesses: `device` because dump_logits never calls
# `.to(device)`, `dtype` from export_onnx.py:154's `model.float().eval()` (stored
# weights are fp16, the reference run is not), `attn` from the sdpa argument at
# dump_logits' build_decision_model call, and `torch_threads` from its
# `torch.set_num_threads(1)`.
#
# Every fixture carries it, including the six that never import torch. They are
# generated by the same pinned environment in the same run, and a header whose shape
# depends on which fixture it heads is a header no single test can check.
            ...
            cases.append(
                {
                    ...
                    # Task 1.7.1. The five tensors the forward actually consumes, so
                    # Task 5.3 (`Collate`) can be asserted without a forward pass and
                    # the fake backend (6.1.2) can key on them. collate_items returns
                    # two more that are deliberately left out: `label`, which is a
                    # constant [-1, -1, -1] because no item here carries one, and
                    # `meta`, which after its key filter restates `qtype` and nothing
                    # else. `target` is absent upstream -- has_target is False.
                    # Shapes are batch-derived, which is the whole point: L and kmax
                    # come from the widest row in the group (common.py:222-223).
                    "collated": {
